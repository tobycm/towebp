...
export function cwebp(input_image: string, output_image: string, options: string, logging = "-quiet"): Promise<string> {
...
export function gwebp(input_image: string, output_image: string, options: string, logging = "-quiet"): Promise<string> {
  // input_image: input image(.jpeg, .pnp ....)
  //output_image: /output image .webp
  //option: options and quality,it should be given between 0 to 100
  const query = `${options} "${input_image}" -o "${output_image}" "${logging}"`; //command to convert image
